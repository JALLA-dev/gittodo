...
Equivalent install commands are `pnpm install -g clerk`, `yarn global add clerk`, `bun add -g clerk`, `brew install clerk/stable/clerk`, or `curl -fsSL https://clerk.com/install | bash`.
...
This setup is linked to the Clerk application `app_3IiGfC0PllxXmmmdjys3sVX10Pt`. Always pass `--app app_3IiGfC0PllxXmmmdjys3sVX10Pt` to `clerk init` so the project links to the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3IiGfC0PllxXmmmdjys3sVX10Pt
```

`clerk init` detects the framework and package manager for existing projects. It installs the correct Clerk SDK and applies framework-specific setup when supported, such as providers, middleware, auth routes, and environment configuration. Do not pass `--framework` or `--pm` for existing projects unless the user explicitly wants to override detection or the CLI asks for those values.

If the directory is empty, ask the user which framework and package manager they want to use. If they have no preference, use Next.js and npm. Then scaffold with explicit framework and package manager options:

```bash
clerk init --framework <framework> --pm <package-manager> --app app_3IiGfC0PllxXmmmdjys3sVX10Pt
```

When choosing a package manager, use the user's preference. If the directory is not truly empty and contains a lockfile or package manager config, use these signals:
...
If no package manager can be detected and the user has no preference, use `npm`.

## Step 4: Verify the Next.js matcher

After `clerk init`, if the project uses Next.js, check `proxy.ts` or `middleware.ts` for Next.js 15 and earlier. Make sure `config.matcher` includes Clerk's auto-proxy path once, after the API/TRPC matcher:
...
### React / Vite specific guidance

For React apps using Clerk with a client-side app shell, use the equivalent Clerk React components such as `SignIn`, `SignUp`, `SignedIn`, `SignedOut`, and `UserButton` from `@clerk/clerk-react` or the framework-specific provider pattern.
...
Use sign-in and sign-up actions when signed out, and a user button when signed in. If clear auth controls already exist, reuse or adapt them instead of duplicating them.
...
- Next.js proxy matchers include `'/__clerk/:path*'` after `'/(api|trpc)(.*)'`
...
- For React/Vite projects, use the React SDK and keep the publishable key only in the client-side environment (`VITE_CLERK_PUBLISHABLE_KEY`), while keeping secret keys on the server only.
...
## React-specific quick implementation notes

For this repository, which is a React + Vite app, the setup should be driven by the React SDK flow rather than the Next.js flow. In practice:

1. Install the Clerk React SDK in the client app.
2. Add the Clerk publishable key to the client environment file.
3. Wrap the app in `ClerkProvider`.
4. Render a visible sign-in/sign-up set when signed out and a `UserButton` when signed in.
5. Confirm that the server-side secret key is never exposed to the browser.
